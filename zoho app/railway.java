...
public class Railway {
    ...
    private static void book_home() {
        System.out.println("\t\t\t\t--------------Welcome to IRTC Traning Booking Application----------");
    ...
    private static void admin() {
        ...
        if (admin_id == 1 && admin_pass == 1234) {
            ...
            System.out.printf("%-20s || %-20s || %-20s || %-20s || \n", "User _Id", "Departure Station",
                    "Arrival Station", "Ticket Number");
            for (int i = 0; i < book_list.size(); i++) {
                System.out.printf("%-20s || %-20s || %-20s || %-20s || \n", book_list.get(i).user_id,
                        train_stops[book_list.get(i).stop],
            ...
            for (int i = 0; i < wait_app.size(); i++) {
                System.out.printf("%-20s || %-20s || %-20s || %-20s || \n", wait_app.get(i).user_id,
                        train_stops[wait_app.get(i).stop],
    ...
    private static void confirm_seat(int no, int stop, int arr) {
        ...
        lb1: for (int j = 0; j < 3; j++) {
            ...
            if (sum == 0) {
                ...
                System.out.println(
                        " Seat Allowcated for User " + user_list.get(index).u_name + " Your Seat Number is " + j +
                                " Your Ticket Number is:" + tic_no);

                System.out.println("Amount for this ticket is " + curr_amt + " Deducted from IRTC Wallet");
                flag = false;
    ...
    private static void wallet() {
        System.out.println("Your IRTC Wallet is " + user_list.get(index).u_amt);
        System.out.println("\t\t\tPress enter to continue...");
        scan.nextLine();
        String s = scan.nextLine();
        System.out.print("\033[H\033[2J");
    ...
    private static void cancel_ticket() {
        ...
        System.out.printf("%-20s || %-20s || %-20s || %-20s || \n", "User _Id", "Departure Station", "Arrival Station",
                "Ticket Number");
    ...
    private static void cancel(int temp_tic) {
        ...
        lab3: for (int i = 0; i < wait_app.size(); i++) {
            if (book_list.get(can_ind).stop <= wait_app.get(i).stop
                    && book_list.get(can_ind).arr >= wait_app.get(i).arr) {
                lb1: for (int j = 0; j < 3; j++) {
                    int sum = 0;
                    ...
                    if (sum == 0) {
                        ...
                        double amt = 0;
                        tic_no++;
                        book_list.add(new waiters(wait_app.get(i).user_id, wait_app.get(i).stop, wait_app.get(i).arr,
                                tic_no));
